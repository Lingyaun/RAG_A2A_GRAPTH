...
void tutorial_complex () {
    ...
    d23_cluster = pipeline->createGGroup<GCluster>({d2, d3}, {d1}, "clusterD23", 2);
    
    
    d_region = pipeline->createGGroup<GRegion>({d1, d23_cluster});    // 创建名为d_region的region信息，并将{d1,d23_cluster,d4}放入其中

    d4 = pipeline->createGNode<MyNode2>(GNodeInfo({d_region}, "nodeD4", 1));
    ...
    // status = pipeline->registerGElement<MyNode2>(&d4, {d_region}, "nodeD4", 1);环检测，存在环status会在执行报错误码
    if (STATUS_OK != status) {
        return;
    }
